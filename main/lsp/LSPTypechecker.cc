...
LSPQueryResult LSPTypechecker::query(const core::lsp::Query &q, const std::vector<core::FileRef> &filesForQuery,
    ...
    // Replace error queue with one that is owned by this thread.
    auto queryFlusher = make_shared<QueryFlusher>();
    gs->errorQueue = make_shared<core::ErrorQueue>(gs->errorQueue->logger, gs->errorQueue->tracer, queryFlusher);
    ...
    gs->lspTypecheckCount++;
    gs->lspQuery = core::lsp::Query::noQuery();
    return LSPQueryResult{move(queryFlusher->queryResponses)};
}
